...
Centre of curvature:: The single reference point from which both Rx and Ry are measured.
...
Ry (vertical radius):: Distance from the centre of curvature to the aperture surface, measured in the vertical plane through the centre of curvature.
A smaller Ry produces a more pronounced vertical (pantoscopic) wrap.
...
The lens body SHALL be curved to match Rx and Ry for its declared size code.
The frame rim aperture SHALL present the same Rx and Ry for its declared size code, so that the lens edge lies flush against the rim channel inner wall around the full perimeter.

NOTE: [informative] Rx and Ry are measured from the shared centre of curvature at the bridge midpoint.
...
The flat lands on each side of the 2.64 mm ridge base are each nominally 0.18 mm wide (= (3.00 − 2.64) / 2).
